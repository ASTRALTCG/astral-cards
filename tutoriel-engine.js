...
(function (root) {
  ...
  function build() {
    ...
    step('Les Cosmiques du Joueur 2', 'La double attaque, L’immunité bestiale, Le déchaînement et Déferlement rejoignent sa zone au cristal, face cachée.', {},()=>['La double atatque',"L'immunité bestiale",'Le déchaînement','Déferlement'].forEach(n=>put(2,10,n,{back:true})));
    step('Les Spéciales du Joueur 2', 'Kaïron souverain, Kaïros souverain et Vairon souverain se placent dans sa zone à l’étoile, face cachée et tournés de 90°.', {},()=>['Kaïron souverain','Kaïros souverrain',C.sovereign].forEach(n=>put(2,1,n,{back:true,tapped:true})));
    ...
    step('Votre Main de départ', 'Chaque joueur pioche cinq cartes. Voici les vôtres : Cruauté des mers, Sous les profondeurs, Geckoto, Falco et Dragon brumeux. La Main adverse reste cachée.', {},()=>{state.phase='Main de départ';draw(1,[C.sea,C.hole,C.gecko,C.falco,C.dragon]);draw(2,['Les écorcheurs','Vargrom',C.urkan,C.assimilation,C.vairon]);});
    ...
    step('Vous piochez Le cyclone', 'Le cyclone rejoint votre Main. Vous avez utilisé une seule de vos deux actions principales.', {},()=>draw(1,[C.cyclone]));
    ...
    step('Phase de pioche', 'Le Joueur 2 pioche une carte. Il prend Pillage bénéfique. Sa Main contient désormais six cartes.', {},()=>{state.phase='Pioche';draw(2,[C.pillage]);});
    ...
    step('Votre pioche du tour 2', 'Vous piochez deux cartes : Démon des tempêtes et L’œil protecteur.', {},()=>{state.phase='Pioche';draw(1,[C.storm,C.eye]);});
    ...
    step('Phase de pioche adverse', 'Le Joueur 2 pioche Valgheim et Silence des arbres.', {},()=>{state.phase='Pioche';draw(2,['Valgheim','Silence des arbres']);});
    ...
    step('Cinq cartes révélées', 'Permutation obscure, Lycanthros, Elphoros, L’éclipse salvatrice et L’appel sauvage sont révélés. Vous pouvez cliquer sur chacune pour la lire. Kaïros n’en fait pas partie : aucune invocation supplémentaire ne se produit.', {reveal:true},()=>{state.revealed=['Permutation obscure','Lycanthros','Elphoros',"L'éclipse salvatrice","L'appel sauvage"];});
    ...
    step('Votre pioche du tour 3', 'Vous piochez La grande main et Roxxor.', {},()=>{state.phase='Pioche';draw(1,[C.hand,C.rox]);});
