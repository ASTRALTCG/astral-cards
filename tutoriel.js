(() => {
  ...
  function exit(){if(document.fullscreenElement)document.exitFullscreen?.();started=false;$('tutorial-game').hidden=true;$('tutorial-start').hidden=false;document.querySelector('.chapter-start[data-chapter="'+chapter+'"]').focus();}
  document.querySelectorAll('.chapter-start').forEach(b=>b.addEventListener('click',()=>start(b.dataset.chapter)));$('restart-tutorial').onclick=()=>start(chapter);$('exit-tutorial').onclick=exit;$('next-step').onclick=()=>current().complete?exit():next();$('previous-step').onclick=()=>{if(index>0){index--;render(true);}};$('focus-card').onclick=()=>showCard(current().focus);$('close-card').onclick=()=>$('card-dialog').close();
